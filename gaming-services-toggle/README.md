...
- Clears Gaming Services' per-user data folder under `%LOCALAPPDATA%\Packages\` — `Remove-AppxPackage` doesn't reliably clean this up on its own.

## What it deliberately keeps

- **Xbox Identity Provider's data folder**, so a cached sign-in can survive between sessions. Flip `PreserveData` in `$Targets` (top of the `.ps1`) to change this. (The durable Microsoft account sign-in most likely lives in the Windows token broker at the OS level, which this script never touches — so you'd probably stay signed in either way.)
- **Framework dependencies** (e.g. `Microsoft.VCLibs`), if installing either package pulled one in. These are shared across other Store apps, so Windows itself never auto-removes one just because a single dependent app was uninstalled — scripting that would risk breaking something unrelated.
